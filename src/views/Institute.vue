<template>
  <v-container>
    <v-row justify="center">
      <v-col cols="12">
<!--        <v-card flat class="mb-5 text-h5 text-decoration-underline"> 사용자 관리 </v-card>-->
        <v-card class="mt-1" flat>
          <v-card-title id="search">
            <v-spacer></v-spacer>
            <v-radio-group v-model="belong" row>
              <v-radio label="전체" :value="1"></v-radio>
              <v-radio label="행정기관" :value="2"></v-radio>
              <v-radio label="공공기관" :value="3"></v-radio>
            </v-radio-group>
            <v-text-field
              v-model="search"
              label="검색어를 입력해주세요."
              append-icon="mdi-magnify"
              hide-details
              dense
              single-line
              class="mr-3"
              outlined
            ></v-text-field>
            <v-col
        class="d-flex"
        cols="1"
      >
        <v-select
              :items="items"
              v-model="selectedValue"
              label="10개"
              hide-details
              dense
              single-line
              class="mr-3"
              outlined
              @change="rowchange()"
        >
          <template #item="{ item }">
            {{ item }}
          </template>
        </v-select>
      </v-col>
            <router-link to="instireg">
            <v-btn color="blue" rounded outlined dark>신규등록<v-icon>mdi-arrow-right-thin</v-icon></v-btn>
            </router-link>
          </v-card-title>
          <v-data-table
            :headers="headers"
            :items="companies"
            :page.sync="page"
            :search.sync="search"
            :items-per-page="itemsPerPage"
            class="pt-3"
            outlined
            :hide-default-footer="true"
            @page-count="pageCount = $event"
          >
            <template #item.name="{ item }">
                <div class="d-flex justify-center">
                {{ item.name }}
                </div>
            </template>
            <template #item.phone="{ item }">
                <div class="d-flex justify-center">
                {{ item.phone }}
                </div>
            </template>
            <template #item.email="{ item }">
              <router-link to="instiinfo">
                <div class="d-flex justify-left font-weight-bold">
                {{ item.email }}
                </div>
              </router-link>
            </template>
            <template #item.website="{ item }">
                <div class="d-flex justify-center">
                {{ item.website }}
                </div>
            </template>
          </v-data-table>
            <v-pagination
            v-model="page"
            :length="pageCount"
            :total-visible="7"
            class="mt-5"
          ></v-pagination>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
</template>

<script>
export default {
  name: "User",
  data() {
    return {
      selectedValue: '',
      search: '',
      page: 1,
      pageCount: 0,
      items: [ '10개', '20개', '30개' ],
      itemsPerPage: 10,
      belong: 1,
      companies: [
        {
          name: "5",
          phone: "1051000",
          email: "기획재정부",
          website: "국가행정조직 중앙행정기관 부",
          location: "현존",
        },
        {
          name: "4",
          phone: "1192000",
          email: "해양수산부",
          website: "국가행정조직 중앙행정기관 부",
          location: "현존",
        },
        {
          name: "3",
          phone: "1250000",
          email: "통일부",
          website: "국가행정조직 중앙행정기관 부",
          location: "현존",
        },
        {
          name: "2",
          phone: "1250000",
          email: "통일부",
          website: "국가행정조직 중앙행정기관 부",
          location: "현존",
        },
        {
          name: "1",
          phone: "1270000",
          email: "법무부",
          website: "국가행정조직 중앙행정기관 부",
          location: "현존",
        },
      ],
    };
  },
  computed: {
    headers() {
      return [
        { text: "NO", value: "name", sortable: false, align: "center"},
        { text: "대표기관코드", value: "phone", sortable: false, align: "center"},
        { text: "전체기관명", value: "email", sortable: false, align: "center"},
        { text: "기관유형", value: "website", sortable: false, align: "center"},
        { text: "폐지구분", value: "location", sortable: false, align: "center"},
      ];
    },
  },
  methods: {
    rowchange: function() {
      console.log(this.selectedValue);
      let str = this.selectedValue;
      this.itemsPerPage = str.substr(0, 2)
    }
  }
}
</script>

<style scoped>
::v-deep table { border-top: 1px solid #000; border-bottom: 1px solid #D1D1D1;}
::v-deep table thead th { font-size: 16px !important; border-top: 3px solid #000;  color: black !important; background-color: #e5f2ff!important;}
::v-deep table td { font-size: 18px !important; }
::v-deep table th:nth-child(2) { width: 20%; }
::v-deep table th:nth-child(3) { width: 35%; }
.v-application a{
  text-decoration: none;
  color: black;
}
#search {
  padding-top: 0;
  padding-bottom: 0;
}
.tables th:nth-child(1) {
  width: 20%;
}
::v-deep .v-btn--outlined {
  border: 3px solid;
}
::v-deep .v-pagination__item {
  box-shadow: none;
}
::v-deep .v-pagination__navigation {
  box-shadow: none;
}
</style>
